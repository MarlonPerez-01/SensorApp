...
const Seleccionar = async (desde, limite) => {
  try {
    const queryAhuachapan = `SELECT id, humedad FROM Ahuachapan ORDER BY id DESC LIMIT ${limite}`;
    const dataAhuchapan = await promisePool.query(queryAhuachapan);

    const queryChalatenango = `SELECT id, humedad FROM Chalatenango ORDER BY id DESC LIMIT ${limite}`;
    const dataChalatenango = await promisePool.query(queryChalatenango);

    const querySantaAna = `SELECT id, humedad FROM SantaAna ORDER BY id DESC LIMIT ${limite}`;
    const dataSantaAna = await promisePool.query(querySantaAna);

    const querySonsonate = `SELECT id, humedad FROM Sonsonate ORDER BY id DESC LIMIT ${limite}`;
    const dataSonsonate = await promisePool.query(querySonsonate);

    const querySanMiguel = `SELECT id, humedad FROM SanMiguel ORDER BY id DESC LIMIT ${limite}`;
